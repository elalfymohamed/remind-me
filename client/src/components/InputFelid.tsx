...
const InputFelid: React.FC<Props> = ({ data, setData, handleSubmit }) => {
  const [isOpen, setIsOpen] = useState<boolean>(false);

  const handleClickOptions = (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    ...
    <div className="add-task">
      ...
      <div className="todo-options">
        <div className="todo-install">
          <div className={`install-btn ${data.install ? "install-active" : ""}`} role={"button"}
            onClick={() => setData({
              ...data,
        ...
        <div className="options-color">
          <div className={`options-color-btn ${isOpen ? "option-active" : ""}`} role={"button"}
